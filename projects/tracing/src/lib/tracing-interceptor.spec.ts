...
import { MockTracer, MockSpan } from 'opentracing/lib/mock_tracer';
...
describe('TracingInterceptorService', () => {
  ...
  it('should trace on an http request', () => {
    ...
    httpMock.expectOne('/endpoint')
      .event(new HttpResponse<any>({ status: 200 }));

    const [span] = tracer.report().spans;

    expect(span.operationName()).toBe('GET /endpoint');
  ...
  it('should skip trace of specified in params', () => {
    http.get('/endpoint', { params: createTracingOptions({ skipTrace: true }) }).subscribe();
  ...
  it('should set correct error tags and log on error', () => {
    ...
    httpMock.expectOne('/endpoint').error(new ErrorEvent('kaki'));
    const [span] = tracer.report().spans;
...
describe('DefaultTracingOptions', () => {
  it('should not trace if enabled=true', () => {
    const { http, httpMock, tracer } = reconfigureTracingModule({ enabled: false });
  ...
  it('should not log when logEvents=true', () => {
    const { http, httpMock, tracer } = reconfigureTracingModule({ logEvents: false });
    http.get('/endpoint').subscribe();
  ...
  it('should set a custom span name when using spanName', () => {
    const { http, httpMock, tracer } = reconfigureTracingModule({
      spanName: () => 'Span Name',
    });
  ...
  it('should skip tracing if skipTrace returns true', () => {
    ...
    httpMock.expectOne('/trace');
    httpMock.expectOne('/no-trace');

    const { spans } = tracer.report();
    expect(spans.length).toBe(1);
    expect(spans[0].operationName()).toContain('trace');
  });
});
...
function reconfigureTracingModule(opts?: DefaultTracingOptions) {
